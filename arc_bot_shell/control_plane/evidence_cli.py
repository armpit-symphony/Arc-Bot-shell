"""Explicit Arc command for a governed redacted Supervisor evidence trace."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Read one persisted, redacted control-plane evidence trace through "
            "mandatory Guardian, LIMA, and Arc acknowledgement. This command "
            "cannot execute the recorded action."
        )
    )
    parser.add_argument("--read", action="store_true", required=True)
    parser.add_argument("--target-request-id", required=True)
