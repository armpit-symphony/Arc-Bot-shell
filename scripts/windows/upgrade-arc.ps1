...
try {
    ...
    if (Test-Path -LiteralPath $releaseRoot -PathType Container) {
        ...
        if ($TestMode) {
            ...
            & $stagingPython -m pip install --disable-pip-version-check "git+https://github.com/armpit-symphony/LIMA-Guardian-Suite.git@$script:GuardianTag"
            if ($LASTEXITCODE -ne 0) {
                throw "Pinned Guardian installation failed for the release."
            }
            & $stagingPython -m pip install --disable-pip-version-check $stagingApp
        }
        if ($LASTEXITCODE -ne 0) {
